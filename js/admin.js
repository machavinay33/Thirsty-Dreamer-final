(() => {
  ...
  function showSetup() {
    setup.hidden = false;
    setup.innerHTML = `<h2>Connect the editor to your Supabase project</h2><p>The website and CMS are ready, but this repository doesn't include your Supabase project URL or public key. Those are needed to sign in, publish edits, save subscribers, and upload media. The public/publishable key belongs in <code>supabase-config.js</code>; never put a service-role or secret key in browser code.</p><ol><li>In Supabase, create an administrator user for your email under <strong>Authentication → Users</strong>.</li><li>Open that user's details and copy the user UUID. In <code>supabase/schema.sql</code>, replace the all-zero UUID in the bootstrap insert with it, then run the script once in <strong>SQL Editor</strong>.</li><li>Set your project's URL and publishable (or legacy anon) key in <code>supabase-config.js</code>, commit and push, then open <a href="/admin.html">/admin.html</a> and sign in.</li></ol><p>Supabase project settings → API contains the project URL and publishable/anon key. A public key is safe here because the SQL enables row-level security; a secret/service_role key is not. See <a href="https://supabase.com/docs/guides/database/postgres/row-level-security" target="_blank" rel="noopener">Supabase RLS guidance</a>.</p>`;
